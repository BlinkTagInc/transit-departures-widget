...
function getTemplatePath(templateFileName, config) {
  const folderPath = config.templatePath === undefined ? path.join(__dirname, '..', 'views/widget/') : path.join(untildify(config.templatePath));

  const filename = `${templateFileName}.pug`;
