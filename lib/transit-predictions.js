...
/*
 * Generate Transit Prediction Tool HTML from GTFS.
 */
module.exports = async config => {
  ...
  return Promise.all(config.agencies.map(async agency => {
    ...
    config.log(`${agencyKey}: Generating Transit Prediction Tool HTML`);
    ...
    // Print stats
    config.log(`${agencyKey}: Transit Prediction Tool HTML created at ${exportPath}`);
