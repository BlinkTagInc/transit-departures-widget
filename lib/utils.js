...
import { getDb, getDirections, getRoutes, getStops, getTrips } from 'gtfs';
import { groupBy, maxBy, size, sortBy, uniqBy } from 'lodash-es';
...
async function getDirectionsForRoute(route) {
  ...
  const directions = await getDirections({ route_id: route.route_id }, [
    'direction_id',
    'direction',
  ]);
  ...
  if (directions.length === 0) {
    ...
    for (const group of Object.values(groupBy(headsigns, 'direction_id'))) {
      ...
      directions.push({
        ...
        direction: `To ${mostCommonHeadsign.trip_headsign}`,
      });
    }
  }
...
async function getStopsForDirection(route, direction) {
  ...
  const stoptimes = await db.all(
    'SELECT stop_id, stop_sequence, trip_id FROM stop_times WHERE trip_id IN (SELECT trip_id FROM trips WHERE route_id = ? AND direction_id = ?) ORDER BY stop_sequence ASC',
  ...
  // Fetch stop details
  const stops = await getStops({ stop_id: sortedStopIds }, [
    'stop_id',
    'stop_name',
    'stop_code',
    'parent_station',
 ...
 */
export async function generateTransitArrivalsWidgetHtml(config) {
  ...
  const sortedRoutes = sortBy(
    sortBy(filteredRoutes, 'route_short_name'),
