...
async function getDirectionsForRoute(route) {
  ...
  const directions = await getDirections({ route_id: route.route_id }, ['direction_id', 'direction']);
...
async function getStopsForDirection(route, direction) {
  ...
  // Fetch stop details
  const stops = await getStops({ stop_id: sortedStopIds }, ['stop_id', 'stop_name', 'stop_code', 'parent_station']);
...
export async function generateTransitArrivalsWidgetHtml(config) {
  ...
  await Promise.all(routes.map(async route => {
    ...
    await Promise.all(directions.map(async direction => {
      ...
      const trips = await getTrips({ route_id: route.route_id, direction_id: direction.direction_id }, ['trip_id']);
  ...
  const parentStationIds = new Set(sortedStops.map(stop => stop.parent_station));
