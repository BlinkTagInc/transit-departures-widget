...
exports.generateTransitArrivalsWidgetHtml = async config => {
  ...
  await Promise.all(routes.map(async route => {
    ...
    if (directions.length === 0) {
      config.logWarning(`Route ${route.route_id} has no directions - removing`);
      return;
    }

    await Promise.all(directions.map(async direction => {
      const stops = await db.all('SELECT DISTINCT stop_times.stop_id, stops.stop_name, stops.stop_code FROM stop_times INNER JOIN stops ON stops.stop_id = stop_times.stop_id WHERE trip_id IN (SELECT trip_id FROM trips WHERE route_id = ?) ORDER BY stop_sequence ASC', [route.route_id]);
